 ...
 * @author weslley
 */
class Marca extends LIController{
    public function subconjunto(array $ids, array $query = []){
        try{
            $response = $this->http->get("marca/set/" . implode(";", $ids), array(
                "headers" => [
    ...
    public function alterar($id, array $payload, array $query = []){
        try{
            $response = $this->http->put("marca/".$id, array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey()),
                ],
                "json" => $payload,
    ...
    public function detalhes($id, array $query = []){
        try{
            $response = $this->http->get("marca/".$id, array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey()),
                ],
                "query" => $query,
    ...
    public function cadastrar(array $payload){
        try{
            $response = $this->http->post("marca", array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey())
                ],
                "json" => $payload
    ...
    public function listarTodas(array $query = []){
        try{
            $response = $this->http->get("marca", array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey())
                ],
                "query" => $query
