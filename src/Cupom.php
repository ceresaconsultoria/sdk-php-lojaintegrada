 ...
 * @author weslley
 */
class Cupom extends LIController{    
    public function alterar($id, array $payload){
        try{
            $response = $this->http->put("cupom/".$id, array(
                "headers" => [
    ...
    public function detalhes($id, array $query = []){
        try{
            $response = $this->http->get("cupom/".$id, array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey()),
                ],
                "query" => $query,
    ...
    public function cadastrar(array $payload){
        try{
            $response = $this->http->post("cupom", array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey())
                ],
                "json" => $payload
    ...
    public function listarTodas(array $query = []){
        try{
            $response = $this->http->get("cupom", array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey())
                ],
                "query" => $query
