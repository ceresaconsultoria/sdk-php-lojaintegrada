 ...
 * @author weslley
 */
class Grade extends LIController{    
    public function cadastrarVariacao($gradeId, array $payload){
        try{
            $response = $this->http->post("grade/".$id."/variacao", array(
                "headers" => [
    ...
    public function detalhes($id, array $query = []){
        try{
            $response = $this->http->get("grades/".$id, array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey()),
                ],
                "query" => $query,
    ...
    public function cadastrar(array $payload){
        try{
            $response = $this->http->post("grades", array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey())
                ],
                "json" => $payload
    ...
    public function listarTodas(array $query = []){
        try{
            $response = $this->http->get("grades", array(
                "headers" => [
                    "Authorization" => sprintf("chave_api %s aplicacao %s", $this->getApiKey(), $this->getAppKey())
                ],
                "query" => $query
